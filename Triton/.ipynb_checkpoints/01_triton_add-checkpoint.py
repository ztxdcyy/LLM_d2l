...
if __name__ == '__main__':
    ...
    # 1. PyTorch 自带加法计时
    # =============================
    torch.cuda.synchronize()  # 保证前面所有 CUDA 操作完成
    ...
    elapsed_torch = start_event.elapsed_time(end_event)  # 毫秒
    print(f"PyTorch add elapsed: {elapsed_torch:.6f} ms")
    # print(output_torch)

    # =============================
    # 2. Triton kernel 加法计时
    # =============================
    torch.cuda.synchronize()
    start_event = torch.cuda.Event(enable_timing=True)
    end_event = torch.cuda.Event(enable_timing=True)
    ...
    torch.cuda.synchronize()
    elapsed_triton = start_event.elapsed_time(end_event)
